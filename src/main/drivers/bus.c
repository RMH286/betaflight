...
bool busWriteRegisterBuffer(const busDevice_t *bus, uint8_t reg, uint8_t *data, uint8_t length) {
...
#ifdef USE_SPI
    case BUSTYPE_SPI:
        return spiBusWriteRegisterBuffer(bus, reg | 0x80, data, length);
#endif
#ifdef USE_I2C
    case BUSTYPE_I2C:
        return i2cBusWriteRegisterBuffer(bus, reg, data, length);
#endif
    default:
        return false;
    }
...
bool busReadRegister(const busDevice_t *busdev, uint8_t reg, uint8_t *data)
{
#if !defined(USE_SPI) && !defined(USE_I2C)
    UNUSED(busdev);
    UNUSED(reg);
    return false;
#else
    return busReadRegisterBuffer(busdev, reg, data, 1);
#endif
}
